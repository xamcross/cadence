...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (path.startsWith('/api/candidate/status/')) {
      ...
      if (path.includes('lighthouse-demo-terminal')) return sendJson(res, 200, cannedStatusTerminal());
      if (path.includes('lighthouse-demo-review')) return sendJson(res, 200, cannedStatusUnderReview());
      if (path.includes(DEMO_TOKEN)) return sendJson(res, 200, cannedStatusPublished());
