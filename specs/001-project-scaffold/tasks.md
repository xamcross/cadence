...
- [X] T023 [US4] Write `fly.toml` at repo root — use Fly Machines-era format (NOT legacy Nomad `[[services]]` format); include: `[http_service]` with `internal_port = 8080`, `force_https = true`, `auto_stop_machines = false`, `auto_start_machines = false`; `[[http_service.checks]]` with `grace_period="60s"`, `interval="10s"`, `method="GET"`, `path="/actuator/health"`, `timeout="5s"`, `port=8081`; `kill_timeout = "35s"` (must exceed Spring's 30 s drain; default 5 s would SIGKILL mid-drain); `[build]` with `dockerfile = "backend/Dockerfile"` (NOTE: `build-context` is not a valid fly.toml key — the Docker build context is the project root, so the Dockerfile uses repo-root-relative `COPY backend/...` paths); `[env]` block with non-secret config: `SERVER_PORT = "8080"`, `MANAGEMENT_SERVER_PORT = "8081"`, `SPRING_PROFILES_ACTIVE = "production"`; ZERO secrets inline — all credentials via `fly secrets set`
  - CORRECTION (2026-06-13): the health check is implemented as a top-level `[checks]` entry (`type="http"`, `port=8081`), NOT `[[http_service.checks]]`. Verified against the current Fly.io fly.toml reference: `[[http_service.checks]]` has no `port` field and only probes `internal_port` (8080), where actuator is not served (403); top-level `[checks]` require an explicit `port` and are the correct way to probe the isolated management port.
...
- [X] T034 [US7] Write `backend/src/main/resources/logback-spring.xml` — emit structured JSON via `LogstashEncoder`; `./gradlew test` stdout contains JSON log lines (required for CI PII grep); confirm no PII fields appear in sample log output by reviewing encoder configuration
  - CORRECTION (2026-06-13): the original config wrote JSON to a `RollingFileAppender` (`logs/cadence.log`) on non-test profiles. That crashed the Fly container at startup — the non-root `cadence` user cannot create `/app/logs`. Replaced with a single JSON `ConsoleAppender` (stdout) for ALL profiles: this is the container-correct 12-factor pattern (Fly + CI capture stdout), keeps the CI PII grep meaningful, and removes the file-permission failure. No file appender.
