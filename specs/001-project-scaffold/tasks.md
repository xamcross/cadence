...
**Sub-agent review**: Completed 2026-06-13 (DevOps + Backend + QA); all critical and non-critical findings applied to plan.md, research.md, data-model.md, contracts/, and quickstart.md before this task list was generated.
...
- [ ] T039 [US7] Make `DeadLetterTest` and `ActuatorPortTest` pass — verify `SecurityConfig` `@Order(1)` chain covers `/actuator/**`; verify `logback-spring.xml` test profile emits JSON to console; confirm `./gradlew test` stdout contains parseable JSON log lines
...
- [ ] T044 [US9] Make `SchedulerCheckpointTest` pass — wire `SchedulerCheckpointService` into `CadenceApplication`; verify `ApplicationReadyEvent` listener fires after context ready (not before); verify replay logic handles concurrent normal-fire + replay scenario without producing duplicate work items; confirm both test methods pass
