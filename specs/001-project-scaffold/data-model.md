...
The changeset class is annotated `@ChangeUnit(id = "001-bootstrap-indexes", order = "001", author = "system")`. It uses `MongoTemplate.indexOps(collectionName).createIndex(new Index().on(field, direction).unique())` for each index entry. `ensureIndex` is deprecated since Spring Data MongoDB 4.5 (removal=true); `createIndex` is the required replacement — it throws `MongoCommandException` on conflicting index definitions, which is correct behavior for a migration (conflict surfaced immediately rather than silently ignored).
