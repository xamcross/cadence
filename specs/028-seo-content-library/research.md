...
**On-disk layout (pinned — review fix)**: each article is emitted as `dist/resources/<slug>/index.html` (directory-index form), and the library index as `dist/resources/index.html`. This is the form Cloudflare Pages serves at the clean URL `/resources/<slug>` (no `.html`), so the self-canonical `/resources/<slug>` matches the served URL exactly with no extension-drop reliance and no canonical/served-URL mismatch. The earlier "`<slug>/index.html` or `<slug>.html`" ambiguity is resolved to the directory form ONLY.
