 ...
 *
 * <p>Every candidate-facing body carries a {@code {{privacy_link}}} footer line (GDPR Art. 14 - the
 * first email is the transparency touchpoint for indirectly-sourced candidates). It is a URL-typed
 * token whose constant value (the public {@code /privacy} page) is injected centrally in
 * {@code EmailTemplateService.renderForSend} (contract C-LINK-4, research D8).
 */
@Component
    ...
    public BuiltInEmailTemplates() {
        defaults.put(EmailMessageType.INVITATION, new Content(
            ...
            "Hi {{candidate_name}},\n\nWe'd like to schedule your {{stage_name}} interview. "
                ...
                + "\n\nView our Privacy Notice: {{privacy_link}}"));
